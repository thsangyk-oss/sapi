...
export default function DroidToolCard({
  ...
  const getConfigStatus = () => {
    ...
    // Check for any SAPI model entry, including legacy custom:9Router entries.
    const currentConfig = droidStatus.settings?.customModels?.find(isSapiCustomModel);
              ...
              <div className="flex flex-col gap-3 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-text-muted">Manual configuration is still available if SAPI is deployed on a remote server.</p>
                  </div>
                </div>
          ...
          {!checkingDroid && droidStatus?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {/* Current configured */}
                {droidStatus?.settings?.customModels?.find(isSapiCustomModel)?.baseUrl && (
