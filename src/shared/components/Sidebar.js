...
const navItems = [
  ...
  { href: "/dashboard/data-codex", label: "Data Codex", icon: "inventory_2" },
  ...
  return (
    <>
        ...
        <div className="px-6 py-4 flex flex-col gap-2">
          <Link href="/dashboard" className="flex items-center gap-3">
            <img
              ...
              className="size-9 shrink-0 rounded-[10px] object-cover shadow-[var(--shadow-warm)]"
            />
            <div className="flex flex-col">
              <h1 className="text-lg font-semibold tracking-tight text-text-main">
                {APP_CONFIG.name}
              </h1>
              <span className="text-xs text-text-muted">{APP_CONFIG.description} · v{APP_CONFIG.version}</span>
            </div>
          </Link>
          {updateInfo && (
      ...
      <ConfirmModal
        ...
        title="Update SAPI"
        message={`This will close SAPI and install v${updateInfo?.latestVersion || ""} in a separate window. Continue?`}
        confirmText="Update"
        cancelText="Cancel"
        variant="primary"
        loading={isUpdating}
...
function UpdateProgress({ status, latestVersion, installCmd, copied, onCopy }) {
  ...
  const steps = [
    { key: "stopped", label: "Stopped SAPI server", state: "done" },
    {
      key: "launched",
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          <h2 className="text-lg font-semibold">
            {done && success ? "Update Completed" : done && !success ? "Update Failed" : "Updating SAPI"}
          </h2>
      ...
      {done && success ? (
        <div className="space-y-2">
          <p className="text-sm text-white/80">
            Run <code className="px-1.5 py-0.5 rounded bg-white/10 text-green-400">start-sapi.cmd</code> in your terminal to start the new version.
          </p>
